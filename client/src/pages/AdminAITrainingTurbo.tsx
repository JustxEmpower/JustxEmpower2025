...
export default function AdminAITrainingTurbo() {
  ...
  const createMutation = trpc.aiTraining.createKnowledge.useMutation({ 
        ...
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          <DialogContent className="max-w-2xl bg-slate-900 border-white/20 text-white">
            ...
            <div className="space-y-4 py-4">
              ...
              <div><Label className="text-slate-300">Answer</Label><Textarea value={formData.answer} onChange={(e) => setFormData({ ...formData, answer: e.target.value })} placeholder="What should the AI respond with? You can enter multiple paragraphs of training content here." rows={12} className="mt-1 bg-white/10 border-white/20 text-white min-h-[200px] resize-y" /><p className="text-xs text-slate-500 mt-1">{formData.answer.length} characters (no limit)</p></div>
            </div>
