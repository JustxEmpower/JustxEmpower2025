...
export default function AdminAnalytics() {
  ...
  const topicDistributionQuery = trpc.aiChatAnalytics.getTopicDistribution.useQuery({ days: 30 });
  const sentimentTrendsQuery = trpc.aiChatAnalytics.getSentimentTrends.useQuery({ days: 30 });
  const chatStatsQuery = trpc.aiChatAnalytics.getConversationStats.useQuery({ days: 30 });
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-2 gap-6 mb-8">
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {topicDistributionQuery.data?.map((topic, index) => (
                      <div
                        key={topic.topic}
                        className="flex items-center justify-between py-2 border-b border-neutral-100 dark:border-neutral-800 last:border-0"
                      >
                    ...
                    {(!topicDistributionQuery.data || topicDistributionQuery.data.length === 0) && (
                      <p className="text-sm text-neutral-500 text-center py-4">
                        No topic data yet
                      </p>
                    )}
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div>
                      <p className="text-sm text-neutral-500 mb-1">Total Conversations</p>
                      <p className="text-2xl font-light text-neutral-900 dark:text-neutral-100">
                        {chatStatsQuery.data?.totalConversations.toLocaleString() || 0}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-neutral-500 mb-1">Total Messages</p>
                      <p className="text-2xl font-light text-neutral-900 dark:text-neutral-100">
                        {chatStatsQuery.data?.totalMessages.toLocaleString() || 0}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-neutral-500 mb-1">Avg. Messages per Conversation</p>
                      <p className="text-2xl font-light text-neutral-900 dark:text-neutral-100">
                        {chatStatsQuery.data?.avgMessagesPerConversation || 0}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-neutral-500 mb-1">Unique Visitors</p>
                      <p className="text-2xl font-light text-neutral-900 dark:text-neutral-100">
                        {chatStatsQuery.data?.uniqueVisitors.toLocaleString() || 0}
                      </p>
                    </div>
                  </div>
                </CardContent>
