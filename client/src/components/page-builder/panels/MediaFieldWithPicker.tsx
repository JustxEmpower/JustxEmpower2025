...
export function MediaFieldWithPicker({
  ...
  const handleMediaSelect = (media: { url?: string; s3Key?: string }) => {
    const mediaUrl = media.url || media.s3Key || '';
    onChange(mediaUrl);
    setShowPicker(false);
  };
    ...
    <div className={`space-y-2 ${className}`}>
      ...
      {showPicker && (
        <MediaPicker
          ...
          filterType={expectedMediaType === 'all' ? undefined : expectedMediaType}
          title={`Select ${mediaTypeLabel}`}
        />
      )}
    </div>
  );
