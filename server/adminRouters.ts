  ...
  pages: router({
    ...
    create: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        // Auto-generate default sections for the new page
        try {
          const db = await getDb();
          if (db) {
            const defaultSections = getDefaultSectionsForPage(input.slug, input.template);
            const sectionValues = defaultSections.map((section, index) => ({
              pageId: page.id,
              sectionType: section.sectionType as any,
              sectionOrder: index,
              title: section.title,
              content: JSON.stringify(section.content || {}),
              requiredFields: section.requiredFields ? JSON.stringify(section.requiredFields) : null,
              isVisible: 1,
            }));
            if (sectionValues.length > 0) {
              await db.insert(schema.pageSections).values(sectionValues);
              console.log(`Auto-created ${sectionValues.length} sections for page ${input.slug}`);
            }
          }
        } catch (error) {
          console.error('Failed to auto-generate sections:', error);
        }
...
// ============================================
// PAGE SECTIONS ROUTER - Section-based content management
// ============================================
...
export const pageSectionsRouter = router({
  ...
  create: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      // Get the max order for this page if not provided
      let order = input.sectionOrder;
      if (order === undefined) {
        const [maxOrder] = await db
          .select({ max: sql<number>`MAX(sectionOrder)` })
          .from(schema.pageSections)
          .where(eq(schema.pageSections.pageId, input.pageId));
      ...
      const [result] = await db
        ...
        .values({
          ...
          requiredFields: input.requiredFields ? JSON.stringify(input.requiredFields) : null,
  ...
  update: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      // Build update object
      const updateObj: any = {};
      if (updateData.sectionType !== undefined) updateObj.sectionType = updateData.sectionType;
      if (updateData.sectionOrder !== undefined) updateObj.sectionOrder = updateData.sectionOrder;
      if (updateData.title !== undefined) updateObj.title = updateData.title;
      if (updateData.content !== undefined) updateObj.content = JSON.stringify(updateData.content);
      if (updateData.requiredFields !== undefined) updateObj.requiredFields = JSON.stringify(updateData.requiredFields);
      if (updateData.isVisible !== undefined) updateObj.isVisible = updateData.isVisible;
  ...
  // Update section content only (for quick edits)
  ...
  reorder: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      for (const item of input) {
        await db
          .update(schema.pageSections)
          .set({ sectionOrder: item.sectionOrder })
          .where(eq(schema.pageSections.id, item.id));
      }
  ...
  getCompleteness: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const requiredFields = section.requiredFields ? JSON.parse(section.requiredFields) : [];
      
      if (requiredFields.length === 0) {
        return { completeness: 100, filledFields: [], missingFields: [] };
      }
      ...
      return { completeness, filledFields, missingFields };
    }),

  // Get completeness for all sections of a page
  getPageCompleteness: publicProcedure
    .input(z.object({ pageId: z.number() }))
    .query(async ({ input }) => {
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database not available" });
      ...
      const sectionResults = sections.map(section => {
        ...
        if (requiredFields.length === 0) {
          return {
            ...
            title: section.title,
            completeness: 100,
            filledFields: Object.keys(content),
            missingFields: [],
          };
        }
      ...
      const totalRequired = sectionResults.reduce((sum, s) => sum + s.filledFields.length + s.missingFields.length, 0);
      const totalFilled = sectionResults.reduce((sum, s) => sum + s.filledFields.length, 0);
      const overallCompleteness = totalRequired > 0 ? Math.round((totalFilled / totalRequired) * 100) : 100;
      
      return {
        sections: sectionResults,
        overallCompleteness,
        totalSections: sections.length,
        completeSections: sectionResults.filter(s => s.completeness === 100).length,
      };
    }),

  // Bulk create sections for a page (for seeding)
  bulkCreate: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const values = input.sections.map(section => ({
        pageId: input.pageId,
        sectionType: section.sectionType as any,
      ...
      await db.insert(schema.pageSections).values(values);
      
      return { success: true, count: values.length };
    }),

  // Auto-sync sections for a page - creates default sections if none exist
  syncSectionsForPage: adminProcedure
    .input(z.object({
      pageId: z.number(),
      pageSlug: z.string(),
    ...
    .mutation(async ({ input }) => {
      ...
      if (existingSections.length > 0) {
        return { success: true, message: 'Sections already exist', created: 0 };
      }
      
      // Define default sections based on page template or slug
      const defaultSections = getDefaultSectionsForPage(input.pageSlug, input.pageTemplate);
      
      // Insert default sections
      const values = defaultSections.map((section, index) => ({
        pageId: input.pageId,
        sectionType: section.sectionType as any,
        sectionOrder: index,
        title: section.title,
      ...
      return { success: true, message: `Created ${values.length} sections`, created: values.length };
    }),

  // Scan and sync all pages - ensures every page has sections
  scanAndSyncAllPages: adminProcedure
    .mutation(async () => {
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database not available" });
      ...
      // Get pages that already have sections
      const pagesWithSections = await db
        .selectDistinct({ pageId: schema.pageSections.pageId })
        .from(schema.pageSections);
      
      const pagesWithSectionsSet = new Set(pagesWithSections.map(p => p.pageId));
      
      // Find pages without sections
      const pagesNeedingSections = allPages.filter(p => !pagesWithSectionsSet.has(p.id));
      
      let totalCreated = 0;
      const results: { pageId: number; slug: string; sectionsCreated: number }[] = [];
      
      for (const page of pagesNeedingSections) {
        const defaultSections = getDefaultSectionsForPage(page.slug, page.template || undefined);
        ...
        if (values.length > 0) {
          ...
          results.push({ pageId: page.id, slug: page.slug, sectionsCreated: values.length });
        }
      }
      ...
      return {
        ...
        pagesWithExistingSections: pagesWithSectionsSet.size,
        pagesUpdated: results.length,
        totalSectionsCreated: totalCreated,
        details: results,
      };
    }),
});

// Helper function to get default sections for a page based on slug or template
function getDefaultSectionsForPage(slug: string, template?: string): Array<{
  sectionType: string;
  title: string;
  content: Record<string, any>;
  requiredFields: string[];
}> {
  // Define section templates for different page types
  const sectionTemplates: Record<string, Array<{
    sectionType: string;
    title: string;
    content: Record<string, any>;
    requiredFields: string[];
    ...
    'home': [
      { sectionType: 'header', title: 'Header Navigation', content: {}, requiredFields: ['navItems', 'logo'] },
      { sectionType: 'hero', title: 'Hero Section', content: {}, requiredFields: ['title', 'subtitle', 'description', 'ctaText', 'ctaLink', 'videoUrl'] },
      { sectionType: 'content', title: 'Philosophy Section', content: {}, requiredFields: ['title', 'description', 'ctaText', 'ctaLink'] },
      { sectionType: 'carousel', title: 'Offerings Carousel', content: {}, requiredFields: ['offerings'] },
      { sectionType: 'community', title: 'Community Section', content: {}, requiredFields: ['title', 'description', 'ctaText', 'ctaLink'] },
      { sectionType: 'footer', title: 'Footer', content: {}, requiredFields: ['copyright', 'socialLinks', 'quickLinks'] },
    ],
    'philosophy': [
      { sectionType: 'header', title: 'Header Navigation', content: {}, requiredFields: [] },
      { sectionType: 'hero', title: 'Hero Section', content: {}, requiredFields: ['title', 'subtitle', 'backgroundImage'] },
      { sectionType: 'content', title: 'Introduction', content: {}, requiredFields: ['headline', 'body'] },
      { sectionType: 'grid', title: 'Core Pillars', content: {}, requiredFields: ['pillars'] },
      { sectionType: 'cta', title: 'Call to Action', content: {}, requiredFields: ['ctaText', 'ctaLink'] },
      { sectionType: 'footer', title: 'Footer', content: {}, requiredFields: [] },
    ],
    'offerings': [
      { sectionType: 'header', title: 'Header Navigation', content: {}, requiredFields: [] },
      { sectionType: 'hero', title: 'Hero Section', content: {}, requiredFields: ['title', 'subtitle', 'backgroundImage'] },
      { sectionType: 'content', title: 'Introduction', content: {}, requiredFields: ['headline', 'description'] },
      { sectionType: 'grid', title: 'Offerings Grid', content: {}, requiredFields: ['offerings'] },
      { sectionType: 'cta', title: 'Get Started CTA', content: {}, requiredFields: ['ctaText', 'ctaLink'] },
      { sectionType: 'footer', title: 'Footer', content: {}, requiredFields: [] },
    ],
    'contact': [
      { sectionType: 'header', title: 'Header Navigation', content: {}, requiredFields: [] },
      { sectionType: 'hero', title: 'Hero Section', content: {}, requiredFields: ['title', 'subtitle'] },
      { sectionType: 'content', title: 'Contact Info', content: {}, requiredFields: ['email', 'phone', 'address'] },
      { sectionType: 'form', title: 'Contact Form', content: {}, requiredFields: ['formFields', 'submitText'] },
      { sectionType: 'social', title: 'Social Links', content: {}, requiredFields: ['instagram', 'linkedin'] },
      { sectionType: 'footer', title: 'Footer', content: {}, requiredFields: [] },
    ],
    'shop': [
      { sectionType: 'header', title: 'Header Navigation', content: {}, requiredFields: [] },
      { sectionType: 'hero', title: 'Hero Banner', content: {}, requiredFields: ['title', 'subtitle', 'backgroundImage'] },
      { sectionType: 'grid', title: 'Product Categories', content: {}, requiredFields: ['categories'] },
      { sectionType: 'products', title: 'Products Grid', content: {}, requiredFields: ['products'] },
      { sectionType: 'footer', title: 'Footer', content: {}, requiredFields: [] },
    ],
    'walk-with-us': [
      { sectionType: 'header', title: 'Header Navigation', content: {}, requiredFields: [] },
      { sectionType: 'hero', title: 'Hero Section', content: {}, requiredFields: ['title', 'subtitle', 'backgroundImage', 'videoUrl'] },
      { sectionType: 'content', title: 'Introduction', content: {}, requiredFields: ['headline', 'description'] },
      { sectionType: 'pricing', title: 'Membership Options', content: {}, requiredFields: ['tiers'] },
      { sectionType: 'testimonials', title: 'Member Testimonials', content: {}, requiredFields: ['testimonials'] },
      { sectionType: 'faq', title: 'FAQ Section', content: {}, requiredFields: ['faqs'] },
      { sectionType: 'cta', title: 'Join CTA', content: {}, requiredFields: ['ctaText', 'ctaLink'] },
      { sectionType: 'footer', title: 'Footer', content: {}, requiredFields: [] },
    ],
    'community-events': [
      { sectionType: 'header', title: 'Header Navigation', content: {}, requiredFields: [] },
      { sectionType: 'hero', title: 'Hero Section', content: {}, requiredFields: ['title', 'subtitle'] },
      { sectionType: 'calendar', title: 'Upcoming Events', content: {}, requiredFields: ['events'] },
      { sectionType: 'gallery', title: 'Past Events Gallery', content: {}, requiredFields: ['gallery'] },
      { sectionType: 'cta', title: 'Host an Event CTA', content: {}, requiredFields: ['ctaText', 'ctaLink'] },
      { sectionType: 'footer', title: 'Footer', content: {}, requiredFields: [] },
    ],
    'resources': [
      { sectionType: 'header', title: 'Header Navigation', content: {}, requiredFields: [] },
      { sectionType: 'hero', title: 'Hero Section', content: {}, requiredFields: ['title', 'subtitle'] },
      { sectionType: 'grid', title: 'Resource Categories', content: {}, requiredFields: ['categories'] },
      { sectionType: 'grid', title: 'Resources List', content: {}, requiredFields: ['resources'] },
      { sectionType: 'cta', title: 'Submit Resource CTA', content: {}, requiredFields: ['ctaText', 'ctaLink'] },
      { sectionType: 'footer', title: 'Footer', content: {}, requiredFields: [] },
    ],
  };
  
  // Default sections for any page type
  const defaultSections = [
    { sectionType: 'header', title: 'Header Navigation', content: {}, requiredFields: [] },
    { sectionType: 'hero', title: 'Hero Section', content: {}, requiredFields: ['title', 'subtitle', 'backgroundImage'] },
    { sectionType: 'content', title: 'Main Content', content: {}, requiredFields: ['body'] },
    { sectionType: 'cta', title: 'Call to Action', content: {}, requiredFields: ['ctaText', 'ctaLink'] },
    { sectionType: 'footer', title: 'Footer', content: {}, requiredFields: [] },
  ];
  
  // Return template-specific sections or default
  return sectionTemplates[slug] || defaultSections;
}
