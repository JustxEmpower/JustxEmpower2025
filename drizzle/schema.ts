...
/**
 * Page sections table for section-based content management
 * Each page can have multiple sections (hero, content, carousel, etc.)
 * Content is stored as JSON with required fields defined per section type
 */
export const pageSections = mysqlTable("pageSections", {
  id: int("id").autoincrement().primaryKey(),
  pageId: int("pageId").notNull(), // References pages.id
  sectionType: mysqlEnum("sectionType", [
    "header", "hero", "content", "carousel", "grid", "form", "video", "quote", 
    "cta", "calendar", "footer", "newsletter", "community", "testimonials", 
    "gallery", "map", "products", "articles", "team", "faq", "pricing", 
    "features", "stats", "social", "rooted-unity", "pillar-grid"
  ]).notNull(),
  sectionOrder: int("sectionOrder").notNull().default(0),
  title: varchar("title", { length: 255 }), // Optional section title for admin reference
  content: text("content").notNull(), // JSON object with section-specific fields
  requiredFields: text("requiredFields"), // JSON array of required field names for this section
...
// Section type definitions with required fields
export const SECTION_TYPE_FIELDS: Record<string, { required: string[]; optional: string[] }> = {
  header: { required: ["logoUrl"], optional: ["navItems", "ctaText", "ctaUrl"] },
  hero: { required: ["title", "backgroundImage"], optional: ["subtitle", "description", "ctaText", "ctaUrl", "videoUrl"] },
  content: { required: ["title", "body"], optional: ["subtitle", "imageUrl", "imagePosition", "ctaText", "ctaUrl"] },
  carousel: { required: ["items"], optional: ["title", "subtitle", "autoplay", "interval"] },
  grid: { required: ["items"], optional: ["title", "subtitle", "columns"] },
  form: { required: ["fields", "submitText"], optional: ["title", "description", "successMessage"] },
  video: { required: ["videoUrl"], optional: ["title", "description", "autoplay", "posterImage"] },
  quote: { required: ["quote"], optional: ["author", "authorTitle", "authorImage"] },
  cta: { required: ["title", "ctaText", "ctaUrl"], optional: ["subtitle", "backgroundImage", "backgroundColor"] },
  calendar: { required: ["events"], optional: ["title", "viewMode"] },
  footer: { required: ["copyright"], optional: ["columns", "socialLinks", "newsletterTitle", "newsletterDescription"] },
  newsletter: { required: ["title", "ctaText"], optional: ["description", "privacyText", "successMessage"] },
  community: { required: ["title"], optional: ["subtitle", "description", "imageUrl", "ctaText", "ctaUrl"] },
  testimonials: { required: ["items"], optional: ["title", "subtitle"] },
  gallery: { required: ["images"], optional: ["title", "subtitle", "columns"] },
  map: { required: ["location"], optional: ["title", "zoom", "markers"] },
  products: { required: ["products"], optional: ["title", "subtitle", "columns"] },
  articles: { required: ["articles"], optional: ["title", "subtitle", "limit"] },
  team: { required: ["members"], optional: ["title", "subtitle"] },
  faq: { required: ["items"], optional: ["title", "subtitle"] },
  pricing: { required: ["plans"], optional: ["title", "subtitle"] },
  features: { required: ["items"], optional: ["title", "subtitle", "columns"] },
  stats: { required: ["items"], optional: ["title", "subtitle"] },
  social: { required: ["links"], optional: ["title"] },
  "rooted-unity": { required: ["title"], optional: ["subtitle", "description", "imageUrl", "ctaText", "ctaUrl"] },
  "pillar-grid": { required: ["pillars"], optional: ["title", "subtitle"] },
};
